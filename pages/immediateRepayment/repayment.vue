...
export default{
	...
	methods: {
		...
		async getBankCards(){
			...
			let res = await this.$api.bindCards({proType:'repayment'},this.userPhoneInfo)				
		...
		async getChannelList(){
			...
			let res = await this.$api.channelList({proType:'repayment'},this.userPhoneInfo)			
			if(res.data.respCode == "SUCCESS" && res.data.dataMap){
				// console.log(res)	
				uni.hideLoading()
		...
		async getcardAuthentication(){
			...
			this.params = {
				...
				rePayCardNo:this.creditListDefault[0].actNo,
			...
			if(res.data.respCode == "SUCCESS" && res.data.dataMap){
				...
				console.log(dataMap)
				uni.redirectTo({
					url:'/pages/immediateRepayment/payTrading?dataMap='+ dataMap
				})								
			}else{		
					uni.hideLoading()
